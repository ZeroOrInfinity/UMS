...
public class JdbcConnectionRepository implements ConnectionRepository {
	...
	@Caching(
		evict = {@CacheEvict(cacheNames = RedisCacheAutoConfig.USER_CONNECTION_HASH_ALL_CLEAR_CACHE_NAME,
							 keyGenerator = "userIdKeyGenerator", beforeInvocation = true),
				 @CacheEvict(cacheNames = RedisCacheAutoConfig.USER_CONNECTION_HASH_ALL_CLEAR_CACHE_NAME,
						     key = "#connection.createData().providerId", beforeInvocation = true),
				 @CacheEvict(cacheNames = RedisCacheAutoConfig.USER_CONNECTION_HASH_CACHE_NAME,
